...
greedyTests = [
  ...
  (5, (\(Alloc a b) -> abs(a-b)) $ alloc (greedy $ greedy $ greedy $ greedy (graph (Alloc 0 0) [1..])), 2) -- This went wrong
  ]
...
optimalTests = [
  ...
  (3, (\(Alloc a b) -> abs(a-b)) $ alloc (optimal (graph (Alloc 0 0) [2,5,6,2,5,4,1,6])), 1), -- This went wrong
  (4, (\(Alloc a b) -> abs(a-b)) $ alloc (optimal (graph (Alloc 0 0) [2,5,6,2,5,4,1,6,2,4,1,4,1,6])), 1) -- This went wrong
  ]
...
metaTests = [
  ...
  (8, (\(Alloc a b) -> abs(a-b)) $ alloc (metaGreedy optimal (metaRepeat 5 greedy) (graph (Alloc 0 0) [6,1,7,2,6])), 2), -- This went wrong
